...
function createRenderer(options) {
     ...
     * @param n2 新的vnode
     * @param container  容器
     * @param parentComponent 父组件   createInstance创建组件实例的时候 用得到
     */
    ...
    function setupRenderEffect(instance, initialVNode, container, anchor) {
        ...
        instance.update = effect(() => {
            ...
            if (!instance.isMounted) {
                ...
                // instance.subtree 用于存储当前组件的render函数生成 vnode?  是否可以称为vnode?
                const subTree = (instance.subTree = instance.render.call(proxy, proxy)); //subTree 虚拟节点树  vnode树
                console.log(subTree);
                patch(null, subTree, container, instance, anchor);
                //element ->mount
                // ! 将render生成的vnode的el 存储到组件的el
                ...
                console.log(subTree);
                instance.subTree = subTree; //将现在的subTree更新到instance.subTree 方便后续再次更新
    ...
    function updateComponent(n1, n2) {
        ...
        if (shouldUpdateComponent(n1, n2)) {
            instance.next = n2;
            instance.update(); // 利用effect返回值runner 调用runner()会再次执行fn
        }
        else {
            n2.el = n1.el;
            instance.vnode = n2;
...
function parseInterpolation(context) {
    ...
    return {
        ...
        content: { type: 1 /* NodeTypes.SIMPLE_EXPRESSION */, content: content }, //"simple_expression"
    };
}
//TEXT类型parse
